...
void AnimationPose::blend(const AnimationPose& pose)
{
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        float sumWeight          = (m_weight.blend_weight[i] + pose.m_weight.blend_weight[i]);
        if (sumWeight != 0)
        {
            float cur_weight          = m_weight.blend_weight[i] / sumWeight;
            m_weight.blend_weight[i]  = sumWeight; 
            bone_trans_one.m_position = Vector3::lerp(bone_trans_one.m_position, bone_trans_two.m_position, 1.f-cur_weight);
            bone_trans_one.m_scale    = Vector3::lerp(bone_trans_one.m_scale, bone_trans_two.m_scale, 1.f-cur_weight);
            bone_trans_one.m_rotation = Quaternion::nLerp(1.f-cur_weight, bone_trans_one.m_rotation, bone_trans_two.m_rotation,true);
        }
    }
}
